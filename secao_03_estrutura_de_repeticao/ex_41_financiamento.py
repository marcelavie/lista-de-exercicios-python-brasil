...
def gerar_dados_de_financiamente(valor_inicial: float):
    ...
    quantidade = 1
    percentual = 0
    print(f'R$ {valor_inicial:.2f}      {percentual}%              {quantidade}                       R$   {valor_inicial:.2f}')
    i = 0
    while i < 5:
        quantidade = 3
        percentual = 0.1 
        for i in range(1, 5):
            divida = valor_inicial + (valor_inicial*percentual)
            print(f'R$ {divida:<12.2f} {percentual:<15.0%} {quantidade:<23d} R$ {divida/quantidade:9.2f}')
            percentual += 0.05
            quantidade += 3
            i += 1 